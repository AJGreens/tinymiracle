...
function EditAnimal() {
  ...
  async function updateAnimal(event) {
    ...
    if (currFosterToken !== prevFosterToken) {
      ...
      const currAnimalFosterRef = ref(
        database,
        "contacts/active/" + prevFosterToken + "/currFostering/" + token
      );
      remove(currAnimalFosterRef);
      if (currFosterName !== "") {
        const currFosterRef = ref(
          database,
          "contacts/active/" + currFosterToken + "/currFostering/" + token
        );
        const allFosterRef = ref(
          database,
          "contacts/active/" +
            currFosterToken +
            "/allFoster/" +
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            update(
              ref(database, animalRefStr + "/pdfs/" + item.name.split(".")[0]),
              { url: downloadURL, name: item.name }
            );
          });
        }
      );
  ...
  function handleDeleteFile(j) {
    if (allFiles[j].url) {
      if (window.confirm("Are you sure you want to delete this item?")) {
        ...
        const deleteFromRTDatabase = ref(
          database,
          "animals/" +
            prevStatus +
            "/" +
      ...
      <div className="container">
        ...
        <Form onSubmit={updateAnimal}>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              onChange={(e) => {
                setMicroChipNum(e.target.value);
              }}
              value={microChipNum}
              type="text"
            />
          ...
          <Form.Group className="mb-3">
            ...
            >
              ...
              <option value="West Highland Terrier (Westie)">
          ...
          <Form.Group className="mb-3">
            ...
            <select
              className="form-select"
              value={currFosterToken + "," + currFosterName}
              name="foster"
              onChange={handleChange}
              aria-label="Default select example"
            >
              ...
              {allFosters.map((foster) => {
                return (
                  <option
                    key={foster.token}
                    value={[foster.token, foster.name]}
                  >
                    {foster.name}
                  </option>
                );
          ...
          <Form.Group className="mb-3">
            <Form.Control
              ...
              accept="image/*"
            />
            <Form.Label htmlFor="uniqueImg" className="btn btn-primary">
              Upload Picture
            </Form.Label>
            <br />
            {imageUrl && (
              <img src={imageUrl} className="adoptableImg" alt="Cute Dog" />
            )}
          </Form.Group>

          <Form.Group>
            {fileCountError !== "" && (
              <Alert variant="danger">{fileCountError}</Alert>
            )}
            <Form.Control
              name="files"
              ...
              multiple={true}
            />
            <Form.Label htmlFor="uniqueFiles" className="btn btn-success">
              Upload Files
            </Form.Label>
            <br />
            {allFiles.map((file, i) => {

...
function ManageContacts() {
          ...
          data.active == null
            ...
            : Object.entries(data.active).map(([key, value]) => {
                return {
                  ...
                  activeFoster: value["activeFoster"],
                };
              });
        let allNonActive =
          data.nonactive == null
            ? []
            : Object.entries(data.nonactive).map(([key, value]) => {
                return {
                  token: key,
                  id: value["id"],
                  name: value["name"],
  ...
  const columns = [
    { dataField: "id", text: "ID", filter: textFilter() },
    {
      ...
      formatter: (cell, row) => (
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            handleUpdateContact(row.activeFoster, row.token);
          }}
        >
          {cell}
        </a>
    ...
    {
      dataField: "activeFoster",
      text: "Active Foster",
      filter: selectFilter({ options: activeFosterSelections }),
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <Button onClick={handleAddContact} className="mb-4">
          Add Contact
        </Button>
        <ToolkitProvider
          keyField="id"
